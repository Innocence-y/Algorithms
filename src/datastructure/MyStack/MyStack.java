 ...
 *
 * @author 闫文强
 * @create 2018-04-01 20:37
 */

/**
 * 这个栈是用数组实现的，内部定义了一个数组，一个表示最大容量的值
 * 以及一个指向栈顶元素的top变量。构造方法根据参数规定的容量创建一个新栈，
 * push()方法是向栈中压入元素，指向栈顶的变量top加一，
 ...
 * 产生的问题：
 ...
 　　②、我们是用数组实现栈，在定义数组类型的时候，也就规定了存储在栈中的数据类型，
       那么同一个栈能不能存储不同类型的数据呢？（声明为Object）
